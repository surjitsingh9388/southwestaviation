...
function appendDashboardPopupData(section, response){
    ...
    if(section == 'dashboard_event_list'){
        ...
        sectionId = 'ptoRequestHistoryModel';
    }
...
$(document).on('click', '.dashboard_event_listing', function(e){
    $('.dashboard_event_heading').html($(this).attr('event-title'));
    $('.dashboard_event_description').html($(this).attr('event-description'));

    $('#displayDashboardEventModel').modal('show');
});

$(document).on('click', '.pto_request_history_btn', function(e){
    var section = 'pto_request_history';
    var url = fetchDashboardPopupURL;
    var dataval = {section:section};
    fetchDashboardPopupDataFromServer(url, dataval, section);
});