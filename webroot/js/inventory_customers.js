...
$(document).on('change keyup', '#otc_invoice_give_discount, #otc_invoice_part_price_each, #otc_invoice_give_discount_percentage', function(e){
    ...
    give_discount_percentage = give_discount_percentage.replace(/\%/g, '');
    
    //give_discount_percentage = parseFloat(give_discount_percentage).toFixed(2);

    var qty_needed = $('#qty-needed').val();
    price_each = qty_needed*price_each;
    var calpercentage = 0;
    if(give_discount_percentage > '0' && $('#otc_invoice_give_discount').is(':checked')){
        calpercentage = (give_discount_percentage*price_each)/100;
        calpercentage = parseFloat(calpercentage).toFixed(2);
    }
    var caltotalprice = price_each-calpercentage;
    if(caltotalprice != '' || caltotalprice != '0.00'){
        caltotalprice = parseFloat(caltotalprice).toFixed(2);

        $('#otc_invoice_part_total_prices').val('$'+caltotalprice);
    }else{
        $('#otc_invoice_part_total_prices').val('$0.00');
    }
});
...
function appendCustomerOTCPopupData(section, response){
    ...
    if(section == 'upload_new_cust_media'){
        ...
        sectionId = 'woMoveItemPhotoModal';
    }else if(section == 'wo_move_item_file_btn'){
        sectionId = 'woMoveItemFileModal';
    }else if(section == 'wo_osr_move_item_btn'){
        sectionId = 'woOSRMoveItemModal';
        ...
        $('#woPrintPreviewCategoryModal').remove();
        $('#woPrintPreviewMaintModal').remove();
        $('#workOrderPrintPreviewModal').remove();
        $('#woPrintPreviewSignoffModal').remove();
        $('#woPrintPreviewDepartmentModal').remove();
        $('#woPrintPreviewDateRangeModal').remove();
        $('#woPrintPreviewWarrantyModal').remove();
        $('#woPrintPreviewLogbookLabelsModal').remove();
        $('#woPPreviewLogbookLabelsPickItemModal').remove();
        $('#woPPLogbookLabelsPrintOptModal').remove();
    }else if(section == 'work_order_print_preview_category'){
        sectionId = 'woPrintPreviewCategoryModal';
        $('#workOrderPrintPreviewModal').remove();
    }else if(section == 'work_order_print_preview_maintenance'){
        sectionId = 'woPrintPreviewMaintModal';
        $('#workOrderPrintPreviewModal').remove();
    }else if(section == 'wo_print_preview_maintenance_bytask'){
        sectionId = 'woPrintPreviewMainByTaskModal';
        $('#workOrderPrintPreviewModal').remove();
    }else if(section == 'wo_print_preview_servicelist_signoff'){
        sectionId = 'woPrintPreviewSignoffModal';
        $('#workOrderPrintPreviewModal').remove();
    }else if(section == 'wo_print_preview_servicelist_department'){
        sectionId = 'woPrintPreviewDepartmentModal';
        $('#workOrderPrintPreviewModal').remove();
    }else if(section == 'wo_print_preview_date_range'){
        sectionId = 'woPrintPreviewDateRangeModal';
        $('#workOrderPrintPreviewModal').remove();
    }else if(section == 'wo_print_preview_warranty'){
        sectionId = 'woPrintPreviewWarrantyModal';
        $('#workOrderPrintPreviewModal').remove();
    }else if(section == 'wo_print_preview_logbook_labels'){
        sectionId = 'woPrintPreviewLogbookLabelsModal';
        $('#workOrderPrintPreviewModal').remove();
    }else if(section == 'wo_pp_logbook_labels_print_option'){
        sectionId = 'woPPLogbookLabelsPrintOptModal';
        $('#woPrintPreviewLogbookLabelsModal').remove();
    }else if(section == 'wo_ppreview_logbook_labels_pickitem'){
        sectionId = 'woPPreviewLogbookLabelsPickItemModal';
        $('#woPPLogbookLabelsPrintOptModal').remove();
    }else if(section == 'wo_pp_logbook_labels_statement'){
        sectionId = 'woPPLogbookPrintStatementModal';
    }
    ...
    if(section == 'work_order_print_preview'){
        var current_item_position = $('#current_item_position').val();
        $('select[name="wo_report_type"] option[value ="40"]').text('Maintenance Printout #'+current_item_position);
        $('select[name="wo_report_type"] option[value ="42"]').text('Maintenance Printout (By Task #'+current_item_position+')');
        $('select[name="wo_report_type"] option[value ="60"]').text('Technicians on Items #'+current_item_position);
        $('select[name="wo_report_type"] option[value ="67"]').text('Time Report (Item #'+current_item_position+', All Tech Logs)');
...
function overrideComplianceBlockDate(){
    $('#engine_due_date, #engine_date_installed, #airframe_due_date, #airframe_date_installed, #inspections_due_date, #inspections_date_override, #date_labeled, #print_prev_date_to_show, #wo_daterange_start_date, #wo_daterange_end_date').datetimepicker({
        format: 'MM-DD-YYYY'
    });
...
function overrideWorkOrderOSRBlockDate(){
    $('#outside_date_due, #vendor_approval_expires, #po_date_order_placed, #po_general_est_arrival_date, #po_warranty_expires, #po_item_shelf_life, #po_item_estimated_arrival_date, #option_gen_info_due_date').datetimepicker({
        format: 'MM-DD-YYYY'
    });
...
$(document).on('blur', '#give_discount_percentage, #owner, #tax_percentage, #labor_discount_percentage, #parts_discount_percentage, #percentage_off_over_cost_val, #otc_invoice_give_discount_percentage, #aircraft-part-discount-percentage, #aircraft-use-specific-rate-hrs-val, #aircraft-labor-discount-percentage', function(e){
    let value = $(this).val().replace(/\%/g, '');
    if(value != ''){
        value = parseFloat(value).toFixed(2);
        $(this).val(value+'%');
...
$(document).on('click', '.close_wo_item_signoff', function(e){
    var btnclickattr = '';
...
var isChanged = false;
$(document).on('input change keyup', '#frmAircraftWorkOrderItems input, #frmAircraftWorkOrderItems textarea, #frmAircraftWorkOrderItems select', function () {
    isChanged = true;
});

$(document).on('input change keyup', '#frmAircraftWorkOrderItemOverviews input, #frmAircraftWorkOrderItemOverviews textarea, #frmAircraftWorkOrderItemOverviews select', function () {
    isChanged = true;
});

$(document).on('input change keyup', '#frmAircraftWorkOrderItemServices input, #frmAircraftWorkOrderItemServices textarea, #frmAircraftWorkOrderItemServices select', function () {
    isChanged = true;
});

$(document).on('click', '.closeWorkOrderDetBtn', function(e){
    if(isChanged == true){
        isChanged = false;
...
$(document).on('click', '.work-order-prev-btn, .work-order-next-btn, .work-order-new-item-btn', function(e){
    ...
    if(work_order_id != '' && work_order_id != undefined){
        ...
        if(isChanged == false){
            getWorkOrderItemDetails(dataval, seltabid);
        }else{
    ...
    $.ajax({
        ...
        success: function (response) {
            if(response == 'failure'){
                ...
                if($('#wo_item_status').val() == '3'){
                    $('.saveAircraftWODetBTN').prop('disabled', true);
                }else{
                    $('.saveAircraftWODetBTN').prop('disabled', false);
                }
            }
        }
    });
...
$(document).on('click', '.wo-go-to-maintenance-info', function(e){
    ...
    if(section != '' && section != undefined){
        var customer_id = $('#wo_customer_id').val();
        var aircraft_id = $('#wo_aircraft_id').val();
        var clicksection = 'work_order';

        var dataval = {section:section, customer_id:customer_id, aircraft_id:aircraft_id, clicksection:clicksection};
...
$(document).on('click', '.saveWOOptionMiscFuelCharges', function(e){
    ...
    if(fuel_price == '' || fuel_price == '0' || fuel_price == '0.00'){
...
$(document).on('click', '.wo-misc-fuel-charges', function(e){
    $('.wo-misc-fuel-charges').removeClass('wo-misc-fuel-charges-active');
    $(this).addClass('wo-misc-fuel-charges-active');
});

$(document).on('click', '.remove-wo-misc-fuel-charges', function(e){
    var currentvar = $(this);
    var misc_fuel_charges_id = currentvar.attr('data-val');
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if(obj.status == 'failure'){
                        ...
                        currentvar.parent().parent().remove();
                    }
                }
            });
        }
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if(obj.status == 'success') {//console.log(obj.conditions);
                    ...
                    var unit_cost = '$'+(obj.inventoryitems.unit_cost != '' && obj.inventoryitems.unit_cost != null ? parseFloat(obj.inventoryitems.unit_cost).toFixed(2) : '0.00');
                    $('#price_each').val(unit_cost);
                    $('#part_total_prices').val(unit_cost);
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if(obj.status == 'failure'){
                    ...
                    $('#signoff_insp_categories').html(obj.signoffinspectionhtml);
                    $('#wo-signoff-tbody').html(obj.signoffitemhtml);
                    $('.saveWOItemSignoffCategory').css('display', 'none');
                    $('#wo_item_signoff_id').val(obj.signoff_id);
                    if(signoff_category == '1'){
                        $('#wo_item_status').val('3');
                        $('.selectpicker').selectpicker('refresh');

                        $('#wo-signoff-tbody tr').each(function(index, tr) { 
                            var active_class=$('.wo-signoff-tr-active');
                            if(active_class.attr('data-val') == signoff_wo_item_id){
                                active_class.find('td:last').html('Finished');
                                active_class.removeClass('wosignoff-color-box2');
...
$(document).on('click', '.clearWOItemSignoffCategory', function(e){
    var inspection_code = $.trim($('#wo_item_signoff_inspection_code').val());
...
$(document).on('change', '#owner_authentication', function(e){
    var owner_authentication = $(this).val();
    var owner_authentication_old = $('#owner_authentication_old').val();
    if(owner_authentication == '3'){
        if(confirm('Are you sure want to change the owner authentication to `No`')){
            var corrective_action = $('#wo-corrective-action').val();
            var authentication_no_txt = ' ...Customer declined any corrective action at this time.';
            var wo_item_id = $('#wo_item_id').val();
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if(obj.status == 'failure') {
                            ...
                            if(obj.corrective_action_count == '0'){
                                corrective_action += authentication_no_txt;
                                $('#wo-corrective-action').val(corrective_action);

                                $('#wo_item_status').val('4');
                                $('.selectpicker').selectpicker('refresh');
                            }else{
                                $('#wo-corrective-action').val(corrective_action);
                            }
            ...
            $('#owner_authentication').val(owner_authentication_old);
        }
    }else{
        $('#owner_authentication_old').val(owner_authentication);
    }
    $('.selectpicker').selectpicker('refresh');
});
...
$(document).on('click', '.continueWOPrintBtn', function(e){
    var wo_report_type = $('#wo_report_type').val();
    if(wo_report_type != '' && wo_report_type != undefined){
        $('.modal-backdrop').remove();

        if(wo_report_type == '11'){
            var section = 'work_order_print_preview_category';
            $('#workOrderPrintPreviewModal').remove();
            ...
            if(section != '' && section != undefined){
                ...
                var dataval = {section:section, customer_id:customer_id, aircraft_id:aircraft_id, wo_item_id:wo_item_id, work_order_id:work_order_id, wo_report_type:wo_report_type};
                
                fetchOTCCustomPopupDataFromServer(section, dataval);
            }
        }else if((wo_report_type >= '12' && wo_report_type <= '22') || (wo_report_type >= '46' && wo_report_type <= '47')){
            var wo_corrective_action = $('#wo-corrective-action').val();
            if($.trim(wo_corrective_action) == ''){
                if(confirm('You have BLANK Corrective Actions. Do you wish to continue?')){
                    validateWOACTT();
                }
            }else{
                validateWOACTT();
            }
        }else if((wo_report_type == '70')){
            if(confirm('Do you want to use a date range?')){
                var section = 'wo_print_preview_date_range';
                $('#workOrderPrintPreviewModal').remove();
        ...
        }else if(wo_report_type >= '39' && wo_report_type <= '40'){
            var section = 'work_order_print_preview_maintenance';
            $('#workOrderPrintPreviewModal').remove();
            $('#woPrintPreviewMainByTaskModal').remove();
            $('#woPrintPreviewMaintModal').remove();
        ...
        }else if(wo_report_type == '42'){
            var section = 'wo_print_preview_maintenance_bytask';
            $('#workOrderPrintPreviewModal').remove();
        ...
        }else if(wo_report_type == '49' || wo_report_type == '56'){
            var section = 'wo_print_preview_servicelist_signoff';
            $('#woPrintPreviewSignoffModal').remove();
        ...
        }else if(wo_report_type == '52'){
            var section = 'wo_print_preview_servicelist_department';
            $('#woPrintPreviewDepartmentModal').remove();
        ...
        }else if(wo_report_type >= '71' && wo_report_type <= '74'){
            checkWOItemOverviewWarranty();
        }else if((wo_report_type == '37')){
            var section = 'wo_print_preview_logbook_labels';
            $('#workOrderPrintPreviewModal').remove();
        ...
        success: function (response) {
            ...
            if(obj.status == 'success') {
                if(obj.is_actt_blank == '0'){
                    if(confirm('There is no A/C TT is set. Please make sure Options > Log Book Values has the currect information.\n\Do you wish to continue?')){
                        var params = $('#frmWOPrintPreview').serialize();
                        downloadPDFAjax(printPreviewWOURL, params);
                    }
                }else{
        ...
        success: function (response) {
            ...
            if(obj.status == 'success') {
                if(obj.warranty_count > '1'){
                    var wo_report_type = $('#wo_report_type').val();
                    var section = 'wo_print_preview_warranty';
                    var wo_item_id = $('#wo_item_id').val();
                    var work_order_id = $('#work_order_id').val();
...
$(document).on('click', '.continueWOPrintCatBtn', function(e){
    var wo_category_id = $('#wo_category_preview').val();
    if(wo_category_id != '' && wo_category_id != undefined){
        var params = $('#frmWOPrintPreview').serialize();
        downloadPDFAjax(printPreviewWOURL, params);
    }
});

$(document).on('click', '.continueWOPrintMaintBtn', function(e){
    var report_date = $('#print_prev_date_to_show').val();
...
$(document).on('click', '.continueWOPrintMaintByTaskBtn', function(e){
    var params = $('#frmWOPrintPreview').serialize();
    downloadPDFAjax(printPreviewWOURL, params);
});

$(document).on('click', '.continueWOPrintSignoffBtn', function(e){
    var wo_signoff = $('#wo_signoff_preview').val();
    if(wo_signoff != '' && wo_signoff != undefined){
        var params = $('#frmWOPrintPreview').serialize();
        downloadPDFAjax(printPreviewWOURL, params);
    }
});

$(document).on('click', '.continueWOPrintDepartmentBtn', function(e){
    var wo_department = $('#wo_department_preview').val();
    if(wo_department != '' && wo_department != undefined){
        var params = $('#frmWOPrintPreview').serialize();
        downloadPDFAjax(printPreviewWOURL, params);
    }
});

$(document).on('click', '.continueWOPrintDateRangeBtn', function(e){
    var start_date = $('#wo_daterange_start_date').val();
    var end_date = $('#wo_daterange_end_date').val();
...
$(document).on('click', '.continueWOPrintWarrantyBtn', function(e){
    var warranty = $('#wo_warranty_preview').val();
    
    if(warranty != '' && warranty != undefined){
        $('#woPrintPreviewWarrantyModal').modal('hide');
...
$(document).on('click', '.wo_option_warranty_info_row', function(e){
    var warranty_id = $(this).attr('data-val');
    if(warranty_id != '' && warranty_id != undefined){
        $('.wo_option_warranty_info_row').removeClass('option_warranty_info_row_active');
        ...
        $.ajax({
            ...
            success: function (response) {
                if($.trim(response) == 'Failed'){
                    ...
                    $('#warranty_info_payment_options').html(response);
                    $('.selectpicker').selectpicker('refresh');
                }
            }
        });
...
$(document).on('click', '.logbooklabelsbox', function(e){
    $('.logbooklabelsbox').removeClass('logbooklabelsbox-active');
    $(this).addClass('logbooklabelsbox-active');
});

$(document).on('dblclick', '.logbooklabelsbox', function(e){
    var wo_report_type = $('input[name=wo_report_type]').val();
    
    var log_book_category = $(this).attr('data-val');
    if(log_book_category != '' && log_book_category != undefined){
        var section = 'wo_pp_logbook_labels_print_option';
        $('#woPrintPreviewLogbookLabelsModal').remove();
        ...
        if(section != '' && section != undefined){
            ...
            var dataval = {section:section, customer_id:customer_id, aircraft_id:aircraft_id, wo_item_id:wo_item_id, work_order_id:work_order_id, wo_report_type:wo_report_type, log_book_category:log_book_category};
            
            $('.modal-backdrop').remove();
            fetchOTCCustomPopupDataFromServer(section, dataval);
        }
    }else{
        alert('Please select Log Book Category');
    }
});

$(document).on('click', '.contLogbooklblprintopt', function(e){
    if($('#labels_mainually_pick_items').is(":checked") && $('#labels_signed_off_items').is(":checked")){
        $('#labels_signed_off_items').prop('checked', false);
        alert("Please select either to manually pick the items, or show the items signed off by a particular user. You cannot select both.");
    ...
    if(statement_name == ''){
        ...
        alert("Please select Statement Name");
        return false;
    }

    if($('#labels_signed_off_items').is(":checked")){
        var labels_technician = $('#labels_technician').val();
        var log_book_category = $('#log_book_category').val();
        var wo_item_id = $('#wo_item_id').val();
        var work_order_id = $('#work_order_id').val();
        var technican_name = $('#labels_technician option:selected').text();
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if(obj.wosignoffitemscount != '0') {
                    ...
                    alert('There were no items found for: log book category "'+log_book_category_name+'", sign-off "Final Inspection", signed off by "'+technican_name+'"');
                }
            }
        });
    }else{
...
function woLabelsPrintOpt(){
    if($('#labels_mainually_pick_items').is(":checked")){
        var section = 'wo_ppreview_logbook_labels_pickitem';
        
        if(section != '' && section != undefined){
            var customer_id = $('#wo_customer_id').val();
            var aircraft_id = $('#wo_aircraft_id').val();

            $('#frmWOPrintPreview').append('<input type="hidden" name="section" value="'+section+'" />');
            $('#frmWOPrintPreview').append('<input type="hidden" name="customer_id" value="'+customer_id+'" />');
            $('#frmWOPrintPreview').append('<input type="hidden" name="aircraft_id" value="'+aircraft_id+'" />');

            var dataval = $('#frmWOPrintPreview').serialize();

            $('#woPPLogbookLabelsPrintOptModal').hide();
            $('#woPPLogbookLabelsPrintOptModal').remove();
...
$(document).on('click', '.selectall_print_logbooklbl', function(e){
    $("input[name='label_wo_item_id[]']").prop('checked', true);
});

$(document).on('click', '.deselectall_print_logbooklbl', function(e){
    $("input[name='label_wo_item_id[]']").prop('checked', false);
});

$(document).on('click', '.continueWOPrintLogBookLblBtn', function(e){
    if($("input[name='label_wo_item_id[]']").prop('checked') == true){
        var params = $('#frmWOPrintPreview').serialize();
        downloadPDFAjax(printPreviewWOURL, params);
    }else{
...
$(document).on('change', '#labels_footer_show', function(e){
    $('.technician_block').css('display', 'none');
    $('.custom_footer_block').css('display', 'none');
    $('#labels_signed_off_items').prop('disabled', true);
    $('#labels_signoffs').prop('disabled', true);

    var selected_technician = 'no tech selected';
    $('.selected_technician').text(selected_technician);
    if($(this).val() != '1' && $(this).val() != '5'){
        $('.technician_block').css('display', '');
    }
    if($(this).val() == '5'){
        $('.custom_footer_block').css('display', '');
    }
    $('.selectpicker').selectpicker('refresh');
});

$(document).on('change', '#labels_technician', function(e){
    $('#labels_signed_off_items').prop('disabled', true);
    $('#labels_signoffs').prop('disabled', true);
    var selected_technician = 'no tech selected';
    if($(this).val() != ''){
        $('#labels_signed_off_items').prop('disabled', false);
        $('#labels_signoffs').prop('disabled', false);

        selected_technician = $('#labels_technician option:selected').text();
    }
    $('.selected_technician').text(selected_technician);
    $('.selectpicker').selectpicker('refresh');
});

$(document).on('click', '.previewstatement', function(e){
    var statement_id = $('#statement_name option:selected').val();
    
    if(statement_id != '' && statement_id != undefined){
        var section = 'wo_pp_logbook_labels_statement';
        var wo_report_type = $('input[name=wo_report_type]').val();
        var wo_item_id = $('#wo_item_id').val();
        var work_order_id = $('#work_order_id').val();
...
$(document).on('click', '.woosrsendmsgbtn', function(e){
    var wo_item_id = $('#wo_item_id').val();
    if(wo_item_id != '' && wo_item_id != undefined){
        var section = 'wo_osr_send_msg_btn';
        
        var customer_id = $('#wo_customer_id').val();
        var aircraft_id = $('#wo_aircraft_id').val();
        var work_order_id = $('#work_order_id').val();
        var work_order_no = $('#work-order-no').val();
        var wo_item_no = $('#current_item_position').val();
        
        var dataval = {section:section, customer_id:customer_id, wo_item_id:wo_item_id, aircraft_id:aircraft_id, work_order_id:work_order_id, work_order_no:work_order_no, wo_item_no:wo_item_no};

        fetchOTCCustomPopupDataFromServer(section, dataval);
    }
});

$(document).on('click', '.woosrmoveitembtn', function(e){
    var wo_item_id = $('#wo_item_id').val();
    if(wo_item_id != '' && wo_item_id != undefined){
        var section = 'wo_osr_move_item_btn';
        
        var customer_id = $('#wo_customer_id').val();
        var aircraft_id = $('#wo_aircraft_id').val();
        var work_order_id = $('#work_order_id').val();
        var wo_item_no = $('#current_item_position').val();
        var osr_info_id = $('.wo-osr-list-active').attr('data-val');
        if(osr_info_id != undefined && osr_info_id != ''){
            var dataval = {section:section, customer_id:customer_id, wo_item_id:wo_item_id, aircraft_id:aircraft_id, work_order_id:work_order_id, wo_item_no:wo_item_no, osr_info_id:osr_info_id};
...
$(document).on('click', '.womovepicturebtn', function(e){
    var wo_item_id = $('#wo_item_id').val();
    if(wo_item_id != '' && wo_item_id != undefined){
        var section = 'wo_move_item_photo_btn';
        
        var customer_id = $('#wo_customer_id').val();
        var aircraft_id = $('#wo_aircraft_id').val();
        var work_order_id = $('#work_order_id').val();
        var wo_item_no = $('#current_item_position').val();
        var wo_move_item_photo_id = $('.wo-item-photo-active').attr('data-val');
        var wo_move_item_photo = $('.wo-item-photo-active').find('td:first').text().trim();

        if(wo_move_item_photo_id != undefined && wo_move_item_photo_id != ''){
            var dataval = {section:section, customer_id:customer_id, wo_item_id:wo_item_id, aircraft_id:aircraft_id, work_order_id:work_order_id, wo_item_no:wo_item_no, wo_move_item_photo_id:wo_move_item_photo_id, wo_move_item_photo:wo_move_item_photo};
...
$(document).on('click', '.womovefilebtn', function(e){
    var wo_item_id = $('#wo_item_id').val();
    if(wo_item_id != '' && wo_item_id != undefined){
        var section = 'wo_move_item_file_btn';
        
        var customer_id = $('#wo_customer_id').val();
        var aircraft_id = $('#wo_aircraft_id').val();
        var work_order_id = $('#work_order_id').val();
        var wo_item_no = $('#current_item_position').val();
        var wo_move_item_file_id = $('.wo-item-file-active').attr('data-val');
        var wo_move_item_file = $('.wo-item-file-active').find('td:first').text().trim();

        if(wo_move_item_file_id != undefined && wo_move_item_file_id != ''){
            var dataval = {section:section, customer_id:customer_id, wo_item_id:wo_item_id, aircraft_id:aircraft_id, work_order_id:work_order_id, wo_item_no:wo_item_no, wo_move_item_file_id:wo_move_item_file_id, wo_move_item_file:wo_move_item_file};
...
$(document).on('click', '.wo-item-osr-prev-btn, .wo-item-osr-next-btn', function(e){
    var clickbtn = $(this).attr('data-val');
...
$(document).on('click', '.add-maint-engine-cylinder', function(e){
    var section = $(this).attr('data-val');
    if(section != '' && section != undefined){
        var customer_id = $('#wo_customer_id').val();
        if(customer_id == '' || customer_id == undefined){
...
$(document).on('click', '.wo-item-part-delete-btn', function(e){
    ...
    if(wo_item_part_id != '' && wo_item_part_id != undefined){
        if(confirm('Delete part number('+part_number+') from this item? This cannot be undone.')){
            ...
            removeWOItemParts(work_order_id, wo_item_id, wo_item_part_id)
        }
    }
});

$(document).on('click', '.wo-item-remove-part-btn', function(e){
    if(confirm("Are you sure want to remove part?")){
        var work_order_id = $('#work_order_id').val();
        var wo_item_part_id = $('.wo-item-part-list-active').attr('data-val');
        if(work_order_id != '' && wo_item_part_id!= '' && wo_item_part_id != undefined){
            var wo_item_id = $('#wo_item_id').val();
