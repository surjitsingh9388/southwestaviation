...
$(document).on('keyup', '#search_txt', function(e){
    ...
    if(search_txt.length <= '1'){
        $('#header_search_list').html('');
    }
});

$(document).on('change', '#header_filter', function(e){
    $('#header_search_list').html('');
    $('#search_txt').val('');
});

$(document).on('click', '#header_search_btn', function(e){
    var search_txt = $('#search_txt').val();
    var search_fld = $('#header_filter').val();
    search_fld = search_fld == 'all' ? '' : search_fld;
    if(search_txt.length >= '3'){
        $.ajax({
            type: "POST",
            url: headerSearchURL,
            data: {search_txt:search_txt, search_fld:search_fld},
            success:function(response) {
                ...
                if(obj.status == 'failure'){
                    ...
                    $('#header_search_list').html(obj.listhtml);
                    $('.searchList').css('display', 'none');
                    if(obj.listhtml != ''){
        ...
        alert("Please type atleast 3 character to search");
    }
});

$(document).on('click', '.loadaircraftworkorder', function(e){
    ...
    if(work_order_no != '' && work_order_no != undefined){
        ...
        $.ajax({
            ...
            async : true,
            success: function (response) {
                $('#aircarftCreateWOModel').remove();
                $("#usertimeclockpopup").append(response);
                $('.selectpicker').selectpicker('refresh');
                $('#aircarftCreateWOModel').modal('show');
            }
        });
    }
});