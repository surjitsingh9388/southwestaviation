...
class AirframeComponentPartsController extends AppController
{
    ...
    public function add()
    {
        ...
        $airCompParts = $this->AirframeComponentParts->newEntity();
        if ($this->request->is('post')) {
            ...
            if ($this->AirframeComponentParts->save($airCompParts)) {
                ...
                $postData['last_revised_by'] = $this->Auth->user('id');
                $partsDetails = $this->airCompLastCwObj->newEntity();
                $partsDetails = $this->airCompLastCwObj->patchEntity($partsDetails, $postData);
                $this->airCompLastCwObj->save($partsDetails);

                //Part installed time
                $partInstalledTimes = $this->partInstTimeObj->newEntity();
        ...
        $this->set(compact('airCompParts', 'actionItems', 'planes', 'ataCode', 'dispArr', 'adsbArr', 'airCPComp'));
    }
    ...
    public function addGroup()
    {
        ...
        if(!empty($params['id'])) {
            ...
            $groupRecord = $this->groupObj->newEntity();
            $groupRecord = $this->groupObj->patchEntity($groupRecord, $params);
            if($this->groupObj->save($groupRecord)) {
    ...
    public function getPartsRecord()
    {
        ...
        if(!empty($params['planeId'])) {
            ...
            /*$childCond = ['AirframeComponentParts.parent_id !='=>0, 'AirframeComponentParts.id !='=>'AirframeComponentParts.parent_id'];
            $childQuery = function ($q) use ($childCond) {
                        return $q->where($childCond)->select(['AirframeComponentParts.id', 'AirframeComponentParts.plane_id', 'AirframeComponentParts.parent_id']);
                    };*/
            $partIds = [''];
