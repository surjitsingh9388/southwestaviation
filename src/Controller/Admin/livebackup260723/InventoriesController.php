<?php
    ...
    class InventoriesController extends AppController
    {
        public function initialize() {
            ...
            $this->loadModel('InventoryHistories');
        ...
        public function ajaxQantitiesSearch(){
            ...
            foreach ( $results as $row){
                $statushtml = $this->Inventory->getInventoriesStatusHTML($row['status']);
        ...
        public function detail($id = null)
        {
            ...
            $inventoryhistories = $this->InventoryHistories->find('all')
                                            ->where(['user_id'=>$this->Auth->user('id'), 'inventory_id'=>$id])
            ...
            $this->set(compact('invenotries', 'actionItems', 'ataCode', 'attachments', 'inventoryhistories', 'install_to', 'installedcomponents', 'transactionhistory'));
        }
                ...
                if(!empty($postData['ids']) && !empty($postData['location_id'])){
                    ...
                    if(empty($errormsg)){
                        $this->Inventories->updateAll(
                            ['location_id' => $postData['location_id'], 'updated_by'=>$this->Auth->user('id')],
                            ['id IN' => $ids]
                        );
        ...
        public function bulkDiscard(){
            ...
            if ($this->request->is(['post'])) {
                ...
                if(!empty($postData['ids'])){
                    ...
                    if(empty($errormsg)){
                        $this->Inventories->updateAll(
                            ['status' => '4', 'discard_reason'=>$postData['reason'], 'location_id'=>'0', 'updated_by'=>$this->Auth->user('id')],
                            ['id IN' => $ids]
                        );
            ...
            if ($this->request->is(['post'])) {
                ...
                if(!empty($postData['ids']) && !empty($postData['install_to'])){
                    ...
                    if(empty($errormsg)){
                        ...
                        $invdataupdate['updated_by'] = $this->Auth->user('id');
                ...
                if(isset($postData['qty']) && $postData['qty'] > $invenotries->qty){
                    ...
                    if($postData['qty'] == $invenotries->qty){
                        ...
                        if(!empty($postData['consume_to'])){
                            ...
                            $inventoryitemsdet = $this->Inventories->patchEntity($inventoryitemsdet, $invitemdata);
        ...
        public function transfer($id=null){
            ...
            if ($this->request->is(['patch', 'post', 'put'])) {
                ...
                if(isset($postData['qty']) && $postData['qty'] > $invenotries->qty){
                    ...
                    $updateval['updated_by'] = $this->Auth->user('id');
                    $qty = $updateval['qty'];
                    $inventoriesdet = $this->Inventories->patchEntity($inventoriesdet, $updateval);
                    if ($this->Inventories->save($inventoriesdet)) {
                        if($postData['qty'] != $invenotries->qty){
                            
                            $copy = $this->Inventories->newEntity();
                            $copy = $this->Inventories->patchEntity($copy, $original);
                            $copy['added_by'] = $this->Auth->user('id');
                            $copy['updated_by'] = $this->Auth->user('id');
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <table>
                                            ...
                                            <tr>
                                                <td colspan="2"></td>
                                                <td><img src="https://chart.googleapis.com/chart?chs=100x100&cht=qr&chl='.$bar_code.'&choe=UTF-8" /></td>
                                            </tr>
                                            <tr>
