<?php
    ...
    class InventoryItemsController extends AppController
    {
        public function initialize() {
            ...
            $this->loadModel('InventoryItemThresholds');
            $this->loadModel('InventoryPartManufacturers');
            $this->loadModel('InventoryPOReceives');
            $this->loadModel('InventoryPOItems');
            $this->loadModel('InventoryItemHistories');
        ...
        public function create()
        {
            ...
            if ($this->request->is('post')) {
                ...
                $inventoryitemdata = $this->InventoryItems->exists(['InventoryItems.part_number'=>$postData['part_number'], 'InventoryItems.status'=>'1']);
        ...
        public function edit($id = null)
        {
            ...
            if ($this->request->is(['patch', 'post', 'put'])) {
                $postData = $this->request->getData();
                //echo "<pre>";print_r( $postData);exit;
                $inventoryitemdata = $this->InventoryItems->exists(['InventoryItems.part_number'=>$postData['part_number'], 'InventoryItems.status'=>'1', 'InventoryItems.id !='=>$id]);
                ...
                if($invenotrypoitems->count() == 0 && $inventories->count() == 0){
                    ...
                    if ($this->InventoryItems->save($inventoryitems)) {
                        if($postData['status'] == '0'){
                            $msg = 'Invenotry items deactivated `'.$inventoryitems->part_number.'`.';
                        }else if($postData['status'] == '1'){
                            $msg = 'Invenotry items activated `'.$inventoryitems->part_number.'`.';
                        }
