<?php
    ...
    class InventoryPurchaseOrdersController extends AppController
    {
        ...
        public function search()
        {
            ...
            $query['count'] = "SELECT count(invpo.id) AS count  FROM `inventory_purchase_orders` as invpo left join inventory_vendors as invpovendor on invpo.vendor = invpovendor.id WHERE 1=1 ";
        ...
        public function create($id=null)
        {
            ...
            $porequestid = isset($this->request->query['porequestid']) ? $this->request->query['porequestid'] : '';
            $invitemid = isset($this->request->query['invitemid']) ? $this->request->query['invitemid'] : '';
            $po_type = isset($this->request->query['po_type']) ? $this->request->query['po_type'] : '';
            ...
            $inventorypurchaseorders = $this->InventoryPurchaseOrders->newEntity();
            $inventorypoitems = $this->InventoryPOItems->newEntity();
            ...
            if ($this->request->is(['patch', 'post', 'put'])) {
                ...
                if(empty($postData['qty']) || count($postData['qty']) == 0){
                    ...
                    for($i=0; $i<count($postData['qty']); $i++){
                        if(isset($postData['inventory_item_id'][$i]) && $postData['inventory_item_id'][$i] == 'Select a part number'){
                            ...
                            $this->Flash->error(__('LineItems['.$i.'] UOM is a required field.'));
                            return $this->redirect( Router::url( $this->referer(), true ) );
                        }
                    }
                ...
                if ($this->InventoryPurchaseOrders->save($inventorypurchaseorders)) {
                    ...
                    if(!empty($postData['request'])){
                        $inventoryrequests = $this->InventoryRequests->newEntity();
                        $inventoryrequests = $this->InventoryRequests->get($postData['request']);
                        $invrequest = [];
                        $invrequest['request_status'] = '5';
                        $invrequest['updated_by'] = $this->Auth->user('id');
                    ...
                    for($i=0; $i<count($postData['uom']); $i++){
                        ...
                        $invrequestpost['uom'] = $postData['uom'][$i];
            ...
            if(!empty($inventorypoid)){
                ...
                $remaingtoaddzero = 6-strlen($inventorypoid);
                $po_number = '';
        ...
        public function detail($id = null)
        {
            ...
            $inventorypohistories = $this->InventoryPurchaseOrderHistories->find('all')
                                    ->where(['user_id'=>$this->Auth->user('id'), 'inventory_purchase_order_id'=>$id])
            ...
            //echo "<pre>";print_r($inventorypurchaseorders);exit;
            $this->set(compact('inventorypurchaseorders', 'actionItems', 'inventorypoitems', 'inventorybillingaddress', 'inventoryshippingaddress', 'countries', 'vendors', 'inventoryporeceivedarr', 'invitmreceived', 'attachments', 'linkorderdata', 'request', 'inventorypohistories'));
        }
        ...
        public function saveInventoryReceived(){
            if (!$this->request->is('ajax')) {
                ...
                $invenotryporeceives = $this->InventoryPOReceives->newEntity();
                $inventoryporecdata = [];
        ...
        public function receive($inventory_po_id, $po_item_id=null){
            ...
            if ($this->request->is(['patch', 'post', 'put'])) {
                ...
                for($i=0; $i<count($postData['inventory_item_id']); $i++){
                    ...
                    $inventorydata['uom']               = 1;
                    $inventorydata['currency']          = 1;
                    $inventorydata['cost']              = $postData['cost'][$i];
                    $inventorydata['account_code']      = $postData['account_code'][$i];
                    $inventorydata['months_new']        = $postData['months_new'][$i];
                    $inventorydata['months_overhaul']   = $postData['months_overhaul'][$i];
                    $inventorydata['months_repair']     = $postData['months_repair'][$i];
                    $inventorydata['hours_new']         = $postData['hours_new'][$i];
                    $inventorydata['hours_overhaul']    = $postData['hours_overhaul'][$i];
                    $inventorydata['hours_repair']      = $postData['hours_repair'][$i];
                    $inventorydata['landings_new']      = $postData['landings_new'][$i];
                    $inventorydata['landings_overhaul'] = $postData['landings_overhaul'][$i];
                    $inventorydata['landings_repair']   = $postData['landings_repair'][$i];
                    $inventorydata['cycles_new']        = $postData['cycles_new'][$i];
                    $inventorydata['cycles_overhaul']   = $postData['cycles_overhaul'][$i];
                    $inventorydata['cycles_repair']     = $postData['cycles_repair'][$i];
            ...
            if ($this->request->is('post')) {
                ...
                if($postData['order_type'] == 'purchase'){
                    ...
                    if(empty($invlinkedorderscount)){
                        $invlinkedorders = $this->InventoryPurchaseOrderLinks->newEntity();
                        $linkorderarr = [];
