<?php
    ...
    class InventoryRepairOrdersController extends AppController
    {
        ...
        protected \App\Model\Table\InventoryRepairOrderHistoriesTable $InventoryRepairOrderHistories;
        ...
        public function initialize():void {
            ...
            $this->InventoryROItems = $this->fetchTable('InventoryROItems');
            $this->InventoryROReceives = $this->fetchTable('InventoryROReceives');
            ...
            $this->InventoryRepairOrderHistories = $this->fetchTable('InventoryRepairOrderHistories');
        ...
        public function ajaxInventoryRepairOrdersearch(){
            ...
            $cond .= $this->InventoryFilter->inventoryRepairFilter($requestData, $queryData);
            ...
            foreach ( $results as $row){//print_r($row);exit;
                ...
                $statushtml = $InventoryStatusHTMLHelper->getRepairOrderStatusHTML($row['status'], $row['ro_status']);
        ...
        public function create($id=null)
        {
            ...
            $InventoryRepairOrders = $this->InventoryRepairOrders->newEmptyEntity();
            $InventoryROItems = $this->InventoryROItems->newEmptyEntity();

            $invid = isset($queryData['invid']) ? $queryData['invid'] : '';
                ...
                if ($this->InventoryRepairOrders->save($InventoryRepairOrders)) {
                    ...
                    for($i=0; $i<count($postData['qty']); $i++){
                        ...
                        /*if(!empty($inventory_id)){
                            $inventories = $this->Inventories->newEmptyEntity();
            ...
            $inventoryrodet = $this->InventoryRepairOrders->find('all', ['order'=>['id asc']])->select(['ro_number'])->all()->last();
            
            if(!empty($inventoryrodet)){
                $currentyear = date('y');
                $inventoryroarr = explode('-', $inventoryrodet->ro_number);
                if($inventoryroarr[0] == $currentyear){
                    $inventorypoid = $inventoryroarr[1]+1;
                    $remaingtoaddzero = 4-strlen($inventorypoid);
                    $ro_number = $inventoryroarr[0].'-';
                    for($i=1; $i<=$remaingtoaddzero; $i++){
                        $ro_number .= '0';
                    }
                    $ro_number .= $inventorypoid;
                }else{
                    $ro_number = $currentyear.'-0001';
                }
            }else{
                $ro_number = $currentyear.'-0001';
            }
        ...
        public function detail($id = null)
        {
            ...
            $inventoryrohistories = $this->InventoryRepairOrderHistories->find('all')
                                    ->where(['user_id'=>$authUserData['id'], 'inventory_repair_order_id'=>$id])
            ...
            $InventoryStatusHTMLHelper = new InventoryStatusHTMLHelper(new \Cake\View\View());
            $statushtml = $InventoryStatusHTMLHelper->getRepairOrderStatusHTML($InventoryRepairOrders->status, $InventoryRepairOrders->ro_status);

            $this->set(compact('InventoryRepairOrders', 'actionItems', 'InventoryROItems', 'inventorybillingaddress', 'inventoryshippingaddress', 'countries', 'vendors', 'attachments', 'invitmreceived', 'inventoryporeceivedarr', 'invitmreceived', 'isallitemreceived', 'inventoryrohistories', 'is_inventory_ro_exist', 'linkorderdata', 'statushtml'));
        }
                ...
                if ($this->InventoryRepairOrders->save($invenotryrepairorders)) {
                    if($ro_status == '3'){
                        ...
                        foreach($inventoryrepairorderitems as $invitm){
                            ...
                            $inventoryarr['updated_by'] = $authUserData['id'];
                            $inventories = $this->Inventories->patchEntity($inventories, $inventoryarr);
        ...
        public function generateInvRODetPdf()
        {
            ...
            $currencyName = !empty($InventoryRepairOrders->currency) ? $currency[$InventoryRepairOrders->currency] : '';
        ...
        public function saveInventoryReceived(){
            if (!$this->request->is('ajax')) {
                ...
                $invenotryporeceives = $this->InventoryROReceives->newEmptyEntity();
                $inventoryporecdata = [];
        ...
        public function receive($inventory_ro_id, $inventory_ro_item_id=null){
            ...
            if ($this->request->is(['patch', 'post', 'put'])) {
                ...
                for($i=0; $i<count($postData['inventory_item_id']); $i++){
                    ...
                    $inventorydata['updated_by']        = $authUserData['id'];
