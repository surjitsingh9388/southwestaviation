...
class PilotsController extends AppController
{
    ...
    protected \App\Model\Table\DutyAssignmentsTable $dutyAssignObj;
    protected \App\Model\Table\PilotCertificatesTable $pilotCertObj;
    protected \App\Model\Table\PilotCheckingsTable $pilotCheckObj;
    protected \App\Model\Table\PilotTrainingsTable $pilotTrainObj;
    protected \App\Model\Table\DutyTimesTable $dutyTimeModel;
    protected \App\Model\Table\DaysOffTable $daysOffModel;
    protected \App\Model\Table\FlightLegDetailsTable $flightLegModel;
    protected \App\Model\Table\DocumentsTable $docsModel;
    ...
    public function initialize():void {
        ...
        $this->dutyAssignObj = $this->fetchTable('DutyAssignments');
        $this->pilotCertObj  = $this->fetchTable('PilotCertificates');
        $this->pilotCheckObj = $this->fetchTable('PilotCheckings');
        $this->pilotTrainObj = $this->fetchTable('PilotTrainings');
    }
    ...
    public function addPilotDutyTime()
    {
        ...
        if(!empty($params['pilot_id'])) {
            $dutyTimeModel = $this->fetchTable('DutyTimes');
    ...
    public function addDaysOffData()
    {
        $params = $this->request->getData();
        if(!empty($params['pilot_id'])) {
            $daysOffModel = $this->fetchTable('DaysOff');
    ...
    public function addFlightLegData()
    {
        $params = $this->request->getData();
        if(!empty($params['pilot_id'])) {
            $flightLegModel = $this->fetchTable('FlightLegDetails');
    ...
    public function getCrewDocuments()
    {
        $params = $this->request->getData();
        if(!empty($params['pilotid'])) {
            $docsModel = $this->fetchTable('Documents');
        ...
        if(!empty($name) && isset($temp)) 
        {
            ...
            if (in_array($ext, $arr_ext)) {
                if(move_uploaded_file($temp, WWW_ROOT . 'documents/' . $setNewFileName)) {
                    $docsModel = $this->fetchTable('Documents');
                    $documents = $docsModel->newEmptyEntity(); 
                    if(!empty($params['document_date'])){
                        if (preg_match("/\d{2}\-\d{2}-\d{4}/", $params['document_date'])) {
                            $params['document_date'] = FrozenTime::createFromFormat("m-d-Y", $params['document_date'])->format("m/d/Y");
                        }
                        $params['document_date'] = FrozenTime::createFromFormat('m/d/Y', $params['document_date']);
                    }
                    $documents = $docsModel->patchEntity($documents, $params);
                    //pr($documents);die;
                    if($docsModel->save($documents)) {
                        //save pilot document to history table
                        $this->AircraftHistory->savePilotDocumentToHistory($documents);
