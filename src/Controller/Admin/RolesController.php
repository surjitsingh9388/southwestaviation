...
class RolesController extends AppController
{
    ...
    public function edit($id = null)
    {
        ...
        if (!empty($permissionRoles)) {
            ...
            //if(in_array($role['id'], $permissionRoles)){
                $roleMenuItemsModel = TableRegistry::get('UserMenuItems');
                $roleMenuItems = $roleMenuItemsModel->find('all')->where(['UserMenuItems.role_id' => $role['id']])->toArray(); 

                if(!empty($roleMenuItems)) {
                    $menuItemsModel = TableRegistry::get('MenuItems');
                    $allMenu = $menuItemsModel->find('all')->where(['MenuItems.parent_id' => 0, 'MenuItems.deleted is null'])->order(['MenuItems.order_by ASC'])->toArray();
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            ...
            if ($this->Roles->exists(['role_name' => $postData['role_name'], 'id !=' => $id])) {
                $this->Flash->error(__('Role already exists.'));
            ...
            if ($this->Roles->save($role)) {
                ...
                $entities = $userMenuItemsModel->newEntities($insertData);
                $userMenuItemsModel->deleteAll(['role_id'=>$role['id']]);
                $userMenuItemsModel->saveMany($entities);
                $this->Flash->success(__('The role has been saved.'));
    ...
    {
        $menuItemsModel = TableRegistry::get('MenuItems');
        $allMenu = $menuItemsModel->find('all')->where(['MenuItems.parent_id' => $parentID, 'MenuItems.deleted is null'])->order(['MenuItems.order_by ASC'])->toArray();
        if(empty($allMenu)){
            $chieldMenu['count']=0;
