...
class AppController extends Controller
{
    ...
    protected \App\Model\Table\MenuItemsTable $menuItemObj;
    ...
    public function initialize():void
    {
        ...
        $this->fetchTable('UserMenuItems');*/
        //$this->fetchTable('UserAircrafts');
        ...
        $this->menuItemObj = $this->fetchTable('MenuItems');
    ...
    public function beforeFilter(EventInterface $event) {
        ...
        $this->Authentication->addUnauthenticatedActions($requestallowed);
        if($this->request->is('ajax') && !in_array($this->request->getParam('action'), $requestallowed)){
    ...
    public function isAuthorized($user) {
        ...
        if ($this->request->getParam('prefix') === 'admin') {
            ...
            if ($user['role'] === 'Admin' || array_key_exists($user['role_id'], $permissionRoles)) {
                $this->set('userLoginEmail', $this->Authentication->user('email'));
            	return true;
            } else {
    ...
    public function sendEmail($data) {
        try {
            ...
            $fromName = $data['compName'];
            $subject = $data['subject'];
            $body = $data['messageData'];
            
            $emailSent = AppMailer::sendEmail($to, $subject, $body, $fromEmail, $fromName);
            if ($emailSent) {
                $status = true;
            } else {
                $status = false;
     ...
     *
     * @$authUserData = $this->Authentication->getResult()->getData();param string|null $description.
     * @return string
     */
    ...
    public function checkAction()
    {
        ...
        $roleID = $authUserData['role_id'];
        $allMenu = $this->menuItemObj->find('all')
            ->select(['UserMenuItems.parent_id', 'MenuItems.id', 'MenuItems.name','UserMenuItems.action_add', 'UserMenuItems.action_edit', 'UserMenuItems.action_view', 'UserMenuItems.action_delete', 'UserMenuItems.action_approve_deny', 'UserMenuItems.action_reopen_work_order', 'UserMenuItems.action_final_inspections', 'UserMenuItems.action_clear_signoff'])
            ->join([
                'UserMenuItems' => [
        ...
        foreach ($allMenu as $key => $value) {
            /*if($value['name'] == 'SWAS' || $value['name'] == 'BVAC' || $value['name'] == 'RJC'){
                $techpubl = $value['name'].' ';
            }
            if(in_array($value['name'], $techpublmenulist)){
                $actionItems[$techpubl.$value['name']] = array('action'=>$value['UserMenuItems']);
            }else{
    ...
    public function condRecords($reports, $params = null)
    {
        ...
        if(!empty($reports)) {
            ...
            foreach ($reports as $row) {
                //echo "<pre>";print_r($row['position_id']);exit;
                $mResults['attachment_count'] = !empty($row['attachment_count']) ? $row['attachment_count'] : '';
                //Parent/child
                $subPArr = [];
                $subCArr = [];
                $parent = '';
                ...
                if(!empty($row['item_type']) && $row['item_type'] == 'INSPECTION') {
                    ...
                    if(!empty($row['ad_sb_status'])) {
                        $itemType .= ' - '.$this->AdsbStatus->adsbTitle($row['ad_sb_status']);
                    }
                } else {
                    $itemType = 'INSPECTION';
                }
                ...
                if($row['airframe_component']['log_book'] == 'Airframe' && !empty($row['position_id'])) {
                    $positions = $this->Position->positionTitle($row['position_id']);
                    $itemType .= ' <span class="lblColor">POS:</span>'.$positions;
                    $itemTypePPT .= ' POS:'.$positions;
                }
