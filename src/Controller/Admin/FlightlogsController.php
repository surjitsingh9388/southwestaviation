...
class FlightlogsController extends AppController
{
    ...
    protected \App\Model\Table\CrewsTable $crewObj;
    protected \App\Model\Table\FlightlogDetailsTable $flDetailObj;
    protected \App\Model\Table\TemplatesTable $templateObj;
    protected \App\Model\Table\TripFilesTable $tripFileObj;
    protected \App\Model\Table\ManifestTable $manifestObj;
    protected \App\Model\Table\PilotsTable $pilotObj;
    protected \App\Model\Table\AircraftDiscrepanciesTable $discpObj;
    ...
    public function initialize():void 
    {
        ...
        $this->crewObj = $this->fetchTable('Crews');
        $this->flDetailObj = $this->fetchTable('FlightlogDetails');
        $this->templateObj = $this->fetchTable('Templates');
        $this->tripFileObj = $this->fetchTable('TripFiles');
        $this->manifestObj = $this->fetchTable('Manifest');
        $this->pilotObj = $this->fetchTable('Pilots');
        $this->discpObj = $this->fetchTable('AircraftDiscrepancies');
    }
    ...
    public function flightlog($tripId=null)
    {
        ...
        if(!empty($tripId)) {
            ...
            foreach ($fls as $key=>$value) {
                ...
                $fls[$key]['flightlog_detail']['fuel_burn'] = is_numeric($startFuel) && is_numeric($endFuel) ? $startFuel - $endFuel : '0';
            }
    ...
    public function saveFLSData()
    {
        ...
        if(!empty($params)) {
            ...
            $authUserData = $this->Authentication->getResult()->getData();
            //Add logged in user
            $params['user_id'] = $authUserData['id'];
            ...
            if(!empty($params['id'])) {
                ...
                $fls = $this->Flightlogs->newEmptyEntity();
            }
            if(!empty($params['leg_date'])){
                $params['leg_date'] = new \Cake\I18n\FrozenDate($params['leg_date']);
            }
            ...
            if ($this->Flightlogs->save($fls)) {
                ...
                if($params['status'] == 'closed') { 
                    //Delete previously added component times
                    //$this->deleteCompTime($params['trip_id']);
                    
                    $flId = $this->airCompTimeObj->find() 
                        ->where(['AirframeComponentTimes.flightlog_id'=>$fls->id])
                        ->select(['id'])
                        ->enableHydration(false)
                        ->first();
                ...
                if(!empty($params['fld_id'])) {
                    ...
                    $flDetails = $this->flDetailObj->newEmptyEntity();
                }
                ...
                if(empty($params['fld_id'])) {
                    //save to flight log detail history
                    $this->AircraftHistory->saveFlightLogDetailsHistory($flDetails);
                }
                ...
                if(!empty($params['crew_member'])) {
                    ...
                    foreach ($params['crew_member'] as $key => $value) {
                        ...
                        $insertData[$key]['pilot_flying']    = !empty($params['pilot_flying'][$key]) ? $params['pilot_flying'][$key] : '';
                        $insertData[$key]['member_type']     = $params['member_type'][$key];
                        if(!empty($params['duty_start_date'][$key])){
                            $insertData[$key]['duty_start_date'] = FrozenTime::createFromFormat('m/d/Y', $params['duty_start_date'][$key]);
                        }
                        if(!empty($params['duty_on'][$key])){
                            $insertData[$key]['duty_on']         = $params['duty_on'][$key];
                        }
                        if(!empty($params['duty_off'][$key])){
                        ...
                        //crew data save in history table
                        $this->AircraftHistory->saveFlightLogCrewHistory($insertData[$key]);
                    }
                ...
                if(!empty($params['mf_id'])) {
                    ...
                    $manifest = $this->manifestObj->newEmptyEntity();
                }
    ...
    //Delete previously added component times
    public function deleteCompTime($tripId)
    {
        $flRes = $this->Flightlogs->find() 
                        ->where(['Flightlogs.trip_id'=>$tripId])
                        ->select(['id'])
                        ->enableHydration(false)
                        ->toArray();
        ...
        if(!empty($flRes)) {
            ...
            $compTimes = array('AirframeComponentTimes.flightlog_id IN'=>$flIds);
            $this->airCompTimeObj->deleteAll($compTimes,false);
        }
    }
    ...
    public function updateCompTimes($flId, $data)
    {
        if(!empty($flId) && !empty($data['plane_id'])) {
            ...
            //Delete previously added component times
            //$compTimes = array('AirframeComponentTimes.flightlog_id'=>$flId);
            //$this->airCompTimeObj->deleteAll($compTimes,false);
            $authUserData = $this->Authentication->getResult()->getData();
            ...
            $taxiOut = !empty($data['taxi_out']) ? date('H:i', strtotime($data['taxi_out'])) : date('H:i');
            $ftArr = array(
            ...
            if(!empty($compDetails['airframe_components'])) {
                foreach ($compDetails['airframe_components'] as $key => $value) {
                    ...
                    //$results[$key]['log_date'] = date('m-d-Y H:i:s');
                    $results[$key]['log_date'] = $data['leg_date'].' '.$taxiOut;
    ...
    //Get flight log data
    public function getFlightLogData_10june()
    {
        $params = $this->request->getData();
        $flHtml = '';
        if(!empty($params['planeId']) && !empty($params['startDt']) && !empty($params['endDt'])) {
            ...
            foreach ($comptimes as $key => $value) {
                ...
                if(!empty($value['flightlog_id'])) {
                    $flres = $this->Flightlog->fldata($value['flightlog_id']);
    ...
    public function getFlightLogData()
    {
        ...
        if(!empty($params['planeId']) && !empty($params['startDt']) && !empty($params['endDt'])) {
            ...
            $flHtml = $this->Flightlog->flReportData($params['planeId'], $startDt, $endDt, $type);
            if(!empty($flHtml)) {
                $result = array('status'=>'success', 'data'=>$flHtml);
                echo json_encode($result);die;
            } else {
    ...
    public function generateFLPdf()
    {
        ...
        if(!empty($params['planeId']) && !empty($params['startDt']) && !empty($params['endDt'])) {
            ...
            $flRes = $this->Flightlog->flReportData($params['planeId'], $startDt, $endDt, $type);
            if(!empty($flRes)) {
                $flHtml = $flRes;
            } else {
            ...
            $mainHtml .= '<table style="width:100%; margin:0 auto; padding:0 0 10px 0; text-align:center; font-size:11px;" cellspacing="0" cellpadding="0">
                                <thead>
                                    <tr>
                                        <th style="font-size: 11px; font-family: Arial, Helvetica, sans-serif;font-weight: bold;padding: 5px;border-bottom: 1px solid #c0c0c0; margin: 0; color:#676a6c;width:10%;">Date/Time</th>
                                        <th style="font-size: 11px; font-family: Arial, Helvetica, sans-serif;font-weight: bold;padding: 5px;border-bottom: 1px solid #c0c0c0; margin: 0; color:#676a6c;width:10%;">Trip ID</th>
                                        <th style="font-size: 11px; font-family: Arial, Helvetica, sans-serif;font-weight: bold;padding: 5px;border-bottom: 1px solid #c0c0c0; margin: 0; color:#676a6c;width:10%;">Crew</th>
                                        <th style="font-size: 11px; font-family: Arial, Helvetica, sans-serif;font-weight: bold;padding: 5px;border-bottom: 1px solid #c0c0c0; margin: 0; color:#676a6c;width:6%;">Depart</th>
                                        <th style="font-size: 11px; font-family: Arial, Helvetica, sans-serif;font-weight: bold;padding: 5px;border-bottom: 1px solid #c0c0c0; margin: 0; color:#676a6c;width:6%;">Arrive</th>
                                        <th style="font-size: 11px; font-family: Arial, Helvetica, sans-serif;font-weight: bold;padding: 5px;border-bottom: 1px solid #c0c0c0; margin: 0; color:#676a6c;width:6%;">Leg Time</th>
                                        <th style="font-size: 11px; font-family: Arial, Helvetica, sans-serif;font-weight: bold;padding: 5px;border-bottom: 1px solid #c0c0c0; margin: 0; color:#676a6c;width:6%;">Fuel Burn</th>
                                        <th style="font-size: 11px; font-family: Arial, Helvetica, sans-serif;font-weight: bold;padding: 5px;border-bottom: 1px solid #c0c0c0; margin: 0; color:#676a6c;width:6%;"># Pax</th>
                                        <th style="font-size: 11px; font-family: Arial, Helvetica, sans-serif;font-weight: bold;padding: 5px;border-bottom: 1px solid #c0c0c0; margin: 0; color:#676a6c;width:5%;">Appr.</th>
                                        <th style="font-size: 11px; font-family: Arial, Helvetica, sans-serif;font-weight: bold;padding: 5px;border-bottom: 1px solid #c0c0c0; margin: 0; color:#676a6c;width:5%;">Type</th>
                                        <th style="font-size: 11px; font-family: Arial, Helvetica, sans-serif;font-weight: bold;padding: 5px;border-bottom: 1px solid #c0c0c0; margin: 0; color:#676a6c;width:5%;">Total Time</th>
                                        <th style="font-size: 11px; font-family: Arial, Helvetica, sans-serif;font-weight: bold;padding: 5px;border-bottom: 1px solid #c0c0c0; margin: 0; color:#676a6c;width:5%;">L/G Cyc</th>
                                        <th style="font-size: 11px; font-family: Arial, Helvetica, sans-serif;font-weight: bold;padding: 5px;border-bottom: 1px solid #c0c0c0; margin: 0; color:#676a6c;width:5%;">Eng 1 Time</th>
                                        <th style="font-size: 11px; font-family: Arial, Helvetica, sans-serif;font-weight: bold;padding: 5px;border-bottom: 1px solid #c0c0c0; margin: 0; color:#676a6c;width:5%;">Eng 2 Time</th>
                                        <th style="font-size: 11px; font-family: Arial, Helvetica, sans-serif;font-weight: bold;padding: 5px;border-bottom: 1px solid #c0c0c0; margin: 0; color:#676a6c;width:5%;">Eng 1 Cyc</th>
                                        <th style="font-size: 11px; font-family: Arial, Helvetica, sans-serif;font-weight: bold;padding: 5px;border-bottom: 1px solid #c0c0c0; margin: 0; color:#676a6c;width:5%;">Eng 2 Cyc</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    '.$flHtml.'
    ...
    public function getFlightCenterData()
    {
        ...
        if(!empty($records)) {
            ...
            foreach ($records as $key => $value) {
                ...
                $routeData[$i]['flight_date'] = !empty($value['leg_date']) ? date('m/d/Y', strtotime($value['leg_date'])) : '';
            ...
            $routeData = array_reverse($routeData);
    ...
    public function changeFLStatus()
    {
        ...
        if(!empty($params['tripId']) && !empty($params['flStatus'])) {
            ...
            $res = $this->Flightlogs->updateAll(
                    ['user_id'=>$authUserData['id'],'status'=>$params['flStatus']],
        ...
        if(!empty($name) && !empty($temp))
        {
            ...
            if (in_array($ext, $arr_ext)) {
                if(move_uploaded_file($temp, WWW_ROOT . 'tripfiles/' . $setNewFileName)) {
                    $tripFiles = $this->tripFileObj->newEmptyEntity(); 
    ...
    {
        $tripId = $this->request->getData()['tripid'];
        if(!empty($tripId)) {
            $resArray = $this->getTripFiles($tripId);
    ...
    public function delClosedLeg()
    {
        ...
        if(!empty($params)) {
            ...
            //Delete crew details
            $crewDetail = array('Crews.flightlog_id'=>$params['flid']);
            $this->crewObj->deleteAll($crewDetail,false);

            //Delete manifest details
            $manifest = array('Manifest.flightlog_id'=>$params['flid']);
            $this->manifestObj->deleteAll($manifest,false);

            //Delete trip flight details
            $flDetails = array('FlightlogDetails.flightlog_id'=>$params['flid']);
            $this->flDetailObj->deleteAll($flDetails,false);

            //Delete trip flight details
            $airCompTime = array('AirframeComponentTimes.flightlog_id'=>$params['flid']);
            $this->airCompTimeObj->deleteAll($airCompTime,false);

            //Delete flightlog
            $fllog = array('Flightlogs.id'=>$params['flid']);
            $this->Flightlogs->deleteAll($fllog,false);
            
            $result = array('status'=>'success', 'message'=>'Flight leg deleted successfully.');
            echo json_encode($result);die;
        } else {
            $result = array('status'=>'failure', 'message'=>'Something went wrong. Please try again.');
            echo json_encode($result);die;
    ...
    public function loadTemplate()
    {
        ...
        if(!empty($params['trip_id']) && !empty($params['leg_date'])) {
            $legdate = new \Cake\I18n\FrozenDate($params['leg_date']);
    ...
    public function changeTripStatus()
    {
        ...
        if(!empty($params['tripId'])) {
            $res = $this->Flightlogs->updateAll(
                    ['user_id'=>$authUserData['id'],'status'=>'closed'],
                    ['trip_id'=>$params['tripId']]
                );
        ...
        if(!empty($flightRes)) {
            ...
            foreach($flightRes as $value) {
                ...
                $results[$tripid][] = [
                    ...
                    'crewMember' => (!empty($value['crews'][0]['crew_member']) ? $this->Pilot->getPilotName($value['crews'][0]['crew_member']) : ''),
