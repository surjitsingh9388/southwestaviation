<?php
    ...
    class InventoryShippingOrdersController extends AppController
    {
        ...
        public function initialize():void {
            ...
            $this->InventoryShippingOrderItems = $this->fetchTable('InventoryShippingOrderItems');
            $this->InventoryShippingOrderReceives = $this->fetchTable('InventoryShippingOrderReceives');
        ...
        public function create($id=null)
        {
            ...
            $po_id = isset($queryData['po_id']) ? $queryData['po_id'] : '';

            $InventoryShippingOrders = $this->InventoryShippingOrders->newEmptyEntity();
            $InventoryShippingOrderItems = $this->InventoryShippingOrderItems->newEmptyEntity();
            if(!empty($po_id)){
                ...
                if ($this->InventoryShippingOrders->save($invenotryrepairorders)) {
                    if($shipping_order_status == '3'){
                        ...
                        $res = $this->InventoryShippingOrderItems->updateAll(
                            array('status'=>$shipping_order_status, 'updated_by'=>$authUserData['id']),
                        ...
                        $res = $this->InventoryShippingOrderItems->updateAll(
                            array('updated_by'=>$authUserData['id']),
                            array('inventory_shipping_order_id' => $id)
                        );
            ...
            if ($this->request->is(['patch', 'post', 'put'])) {
                ...
                for($i=0; $i<count($postData['received']); $i++){
                    ...
                    if(!empty($postData['received'][$i])){
                        ...
                        $inventorysorecdata['updated_by'] = $authUserData['id'];
                        $status = '1';
