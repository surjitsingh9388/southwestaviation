...
class InventoryFilterComponent extends Component {
    public function inventoryRequestFilter($requestData, $queryData=[]){
        ...
        //if(!empty($requestData['applyfilter'])){
            ...
            if(!empty($queryData['open_request'])){
                $cond.=" AND invreq.request_status in ('0', '1') AND invreq.status = '1'";
            }
            if(!empty($queryData['past_due'])){
    ...
    public function inventoryPurchaseOrderFilter($requestData, $queryData=[]){ 
        $cond = "";
        
        if(!empty($queryData['inventoryitemid'])){
            $requestData['searchPOItem'] = $queryData['inventoryitemid'];
        }else if(!empty($queryData['open_po'])){
            $requestData['open_po'] = '1';
        }else if(!empty($queryData['open_exchange'])){
            $requestData['open_exchange'] = $queryData['open_exchange'];
        }
        ...
        //if(!empty($requestData['applyfilter'])){
            ...
            if(isset($requestData['po_status']) && $requestData['po_status'] != ''){
                $cond.=" AND invpo.po_status = '".$requestData['po_status']."'";
            }
            if(isset($requestData['exchange_status']) && $requestData['exchange_status'] != ''){
    ...
    public function inventoryRepairFilter($requestData, $queryData=[]){
        $cond = "";
    ...
    public function completeInventoryFilter($requestData, $queryData=[]){
        $cond = "";
        ...
        if(!empty($queryData['out_for_repair'])){
            $cond.=" AND inv.status = '".$queryData['out_for_repair']."'";
        }

        return $cond;
    }

    public function expiringInventoryFilter($requestData, $queryData=[]){
        ...
        if(!empty($requestData['expiration_date_start']) || !empty($requestData['expiration_date_end'])){
            ...
            $pagesource = isset($queryData['from']) ? $queryData['from'] : '';
            if($pagesource == 'exprd'){
    ...
    public function inventoryCustomersFilter($requestData){
        ...
        if( isset($requestData['searchItem']) && !empty($requestData['searchItem'])){
            $search = $requestData['searchItem'];
            $cond.=" AND ( customer_name LIKE '%".$search."%' OR  name2 LIKE '%".$search."%' OR  city LIKE '%".$search."%' OR  cellular_phone LIKE '%".$search."%')";
        }
