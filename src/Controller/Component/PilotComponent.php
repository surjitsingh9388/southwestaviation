...
class PilotComponent extends Component {
    public array $components = ['AircraftHistory'];

    protected \App\Model\Table\DaysOffTable $DaysOff;
    protected \App\Model\Table\DutyTimesTable $DutyTimes;
    protected \App\Model\Table\FlightLegDetailsTable $FlightLegDetails;
    ...
    {
        $pilotModel = $this->getController()->fetchTable('Pilots');
        $pilot = $pilotModel->patchEntity($pilot, $postData);

        if ($pilotModel->save($pilot)) {
            $pilot_id = !empty($postData['pilot_id']) ? $postData['pilot_id'] : '';
            $postData['pilot_id'] = $pilot->id;

            //save to pilot history table
            if(empty($pilot_id) && $type == 'add'){
                $this->AircraftHistory->savePilotHistory($postData);
            }else{
                $this->AircraftHistory->updatePilotHistory($postData);
            }

            //Save Duty Assignment data
            $dutyAssignModel = $this->getController()->fetchTable('DutyAssignments');
            $dutyAssignModel->saveDutyAssignmentData($postData);

            //Save Pilot Certificates data
            $pilotCertModel = $this->getController()->fetchTable('PilotCertificates');
            $pilotCertModel->savePilotCertificateData($postData);

            //Save Pilot Checking data
            $pilotCheckModel = $this->getController()->fetchTable('PilotCheckings');
            $pilotCheckModel->savePilotCheckingData($postData);

            //Save Pilot Training data
            $pilotTrainModel = $this->getController()->fetchTable('PilotTrainings');
    ...
    public function getYNextDue($frequency=null, $lastCompleted=null) {
        $res = '';
        $frequency = !empty($frequency) ? (int)trim($frequency) : 0;
    ...
    public function getPrevDateOff($pilotId, $prevDate)
    {
        ...
        if(!empty($pilotId) && !empty($prevDate)) {
            $daysOffModel = $this->getController()->fetchTable('DaysOff');
    ...
    public function getPrevDateDuty($pilotId, $prevDate)
    {
        $result = [];
        if(!empty($pilotId) && !empty($prevDate)) {
            $dutyTimeModel = $this->getController()->fetchTable('DutyTimes');
    ...
    public function getCurrentDateFL($pilotId, $date)
    {
        ...
        if(!empty($pilotId) && !empty($date)) {
            $flightLegModel = $this->getController()->fetchTable('FlightLegDetails');
