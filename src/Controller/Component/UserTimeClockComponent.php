...
class UserTimeClockComponent extends Component {
    ...
    public function validateUserAccessCode($time_clock_code){
        ...
        $is_valid_time_clock_code = $this->Users->find('all')->where(['id'=>$this->Auth->user('id'), 'time_clock_code'=>$time_clock_code])->count();

        return $is_valid_time_clock_code;
    }

    public function getUserTimeClockDetail(){
        $this->UserTimeClocks = TableRegistry::get('UserTimeClocks');

        $usertimeclocks = $this->UserTimeClocks->find('all')->where(['user_id'=>$this->Auth->user('id'), 'DATE(in_time)'=>date('Y-m-d')])->select($this->UserTimeClocks)->last();
    ...
    public function getAllTimeClockActiveUsers(){
        ...
        $usertimeclocks = $connection->execute(
            "SELECT tc.id, tc.in_time, (CASE WHEN tc.out_time is not NULL THEN (ROUND(SUM(TIMESTAMPDIFF(SECOND, tc.in_time, tc.out_time )) / 3600, 2)) ELSE 0 END) as totaltime, u.full_name FROM `user_time_clocks` tc join users u on tc.user_id = u.id WHERE DATE(tc.in_time) = '".$todaydate."' group by tc.user_id order by tc.in_time",
    ...
    public function filterUserTimeClock($filter){
        ...
        $usertimeclocks = $this->UserTimeClocks->find('all')->where($filter)->select($this->UserTimeClocks);
    ...
    public function filterTimeClockUserLogs($postData){
        ...
        $usertimeclocks = $connection->execute(
            "SELECT tc.id, tc.in_time, tc.out_time, tc.user_id, (CASE WHEN tc.out_time is not NULL THEN (ROUND(TIMESTAMPDIFF(SECOND, tc.in_time, tc.out_time ) / 3600, 2)) ELSE 0 END) as totaltime, u.full_name FROM `user_time_clocks` tc join users u on tc.user_id = u.id WHERE DATE(tc.in_time) between '".$datefrom."' and '".$dateto."'".$wherecond." order by DATE(tc.in_time), u.full_name",
            ['created' => 'datetime'])->fetchAll('assoc');
        //print_r($usertimeclocks);exit;
        return $usertimeclocks;
    }
    ...
    public function getAllUsersTimeClockDateRange($postData){
        ...
        $usertimeclocks = $connection->execute(
            "SELECT u.full_name, (select ROUND(SUM(TIMESTAMPDIFF(SECOND, in_time, out_time )) / 3600, 2) from user_time_clocks where user_id = u.id and DATE(in_time) BETWEEN '".$fromdate."' and '".$todate."' and out_time != '') as totaltime FROM users u order by u.full_name",
            ['created' => 'datetime'])->fetchAll('assoc');
        //print_r($usertimeclocks);exit;
        return $usertimeclocks;
    }
    ...
    public function getAllUsersTimeClockReportSummary($report_date){
        ...
        $usertimeclocks = $connection->execute(
            "SELECT u.full_name, (select ROUND(SUM(TIMESTAMPDIFF(SECOND, in_time, out_time )) / 3600, 2) from user_time_clocks where user_id = u.id and DATE(in_time) = '".$report_date."' and out_time != '') as totaltime FROM users u order by u.full_name",
            ['created' => 'datetime'])->fetchAll('assoc');
        //print_r($usertimeclocks);exit;
        return $usertimeclocks;
    }
    ...
    public function getAllEmpTimeClockDetail($report_date){
        ...
        $usertimeclocks = $connection->execute(
            "SELECT tc.id, tc.in_time, tc.out_time, tc.user_id, (CASE WHEN tc.out_time is not NULL THEN (ROUND(TIMESTAMPDIFF(SECOND, tc.in_time, tc.out_time ) / 3600, 2)) ELSE 0 END) as totaltime, u.full_name FROM `user_time_clocks` tc join users u on tc.user_id = u.id WHERE DATE(tc.in_time) = '".$report_date."' and tc.out_time != '' order by DATE(tc.in_time), u.full_name",
            ['created' => 'datetime'])->fetchAll('assoc');
        //print_r($usertimeclocks);exit;
        return $usertimeclocks;
    }
    ...
    public function allEmpDateRangeSummaryReport($postData){
        ...
        if(isset($usertimeclocklist[0]['full_name'])){
            ...
            foreach($usertimeclocklist as $clock){
                $mainHtml .= '<tr>
                                <td>'.$clock['full_name'].'</td>
                                <td align="right">'.(!empty($clock['totaltime']) ? $clock['totaltime'] : '0.00').'</td>
    ...
    public function getTimeClockDetailByUser($fromdate, $todate, $user_id){
        ...
        for ( $i = $startTime; $i <= $endTime; $i = $i + 86400 ) {
            ...
            $timeclocks = $connection->execute(
                "SELECT tc.id, tc.in_time, tc.out_time, tc.user_id, (CASE WHEN tc.out_time is not NULL THEN (ROUND(TIMESTAMPDIFF(SECOND, tc.in_time, tc.out_time ) / 3600, 2)) ELSE 0 END) as totaltime FROM `user_time_clocks` tc WHERE DATE(tc.in_time) = '".$thisDate."' and tc.user_id = '".$user_id."' group by tc.in_time order by DATE(tc.in_time) desc",
                ['created' => 'datetime'])->fetchAll('assoc');
