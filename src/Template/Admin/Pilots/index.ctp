...
<script type="text/javascript">
    function action(e){
        ...
        if(confirm(msg)){
            ...
            var pilot_id = $(e).attr("data-pilot_id");
            $("#actionForm").attr("action",url);
            $("#actionForm").append("<input type='hidden' name='id' value='"+pilot_id+"'/>");
            $("#actionForm").submit();
         }
         return false;  
    }
    $(document).ready(function() {
        var dataTable = $('#datatable').DataTable( {
            ...
            "ajax":{
                url:"<?php echo $this->Url->build(['controller'=>'Pilots', 'action'=>'ajaxManagePilotsSearch']); ?>",
...
<div class="content sliding">
    <div class="outerWrapper">
        <div class="btnWrapper">
            <h2 class="heading">Pilots List</h2>
            <?php
            if((!empty($actionItems) && $actionItems['action']['action_add']==1) || $sessionUser['id'] == 1) {
                echo $this->Html->link("<i class='fa fa-plus'></i> Add Pilot", array('action' => 'add'), array('class' => 'btn btn-default', 'escape' => false));
            }
            ?>
        </div>
                ...
                <table id="datatable" class="table dataTable" width="100%">
                    <thead>
                        <tr>
                            ...
                            <th style="vertical-align: top;" scope="col"><?php echo __('Email'); ?></th>
                            <th style="vertical-align: top;" scope="col"><?php echo __('Certificate Number'); ?></th>
                            <th style="vertical-align: top;" scope="col"><?php echo __('Phone'); ?></th>
                            <th style="vertical-align: top;" scope="col"><?php echo __('Role'); ?></th>
