 ...
 */
class DutyTimesTable extends Table
{
    /**
     * Initialize method
     *
    ...
    public function savePilotDutyTimesData($postData)
    {
        if(!empty($postData)) {
            if (!empty($postData['id'])) {
                $dutyTimes = $this->get($postData['id']);
            } else {
                $dutyTimes = $this->newEmptyEntity();
            }
            if(!empty($postData['selected_date'])){
                $postData['selected_date'] = FrozenTime::createFromFormat('m/d/Y', $postData['selected_date']);
            }
            
            $dutyTimes = $this->patchEntity($dutyTimes, $postData);//echo "<pre>";print_r($dutyTimes);exit;
            if($this->save($dutyTimes)) {
                return true;
            }
            return false;
        }
