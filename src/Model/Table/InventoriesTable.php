...
class InventoriesTable extends Table
{
    ...
    public function beforeSave($options = array())
    {
        ...
        if(!empty($entity->expiration)) {
            $entity->expiration = $service->dateFormatBeforeSave($entity->expiration);
        }

        if(!empty($entity->received)) {
            $entity->received = $service->dateFormatBeforeSave($entity->received);
        }

        if(!empty($entity->warranty_expire)) {
            $entity->warranty_expire = $service->dateFormatBeforeSave($entity->warranty_expire);
        }
        ...
        if(!empty($entity->id)){
            ...
            $InventoriesModel = FactoryLocator::get('Table')->get('Inventories');
            $inventories = $InventoriesModel->get($entity->id);
            $InventoryHistoriesModel = FactoryLocator::get('Table')->get('InventoryHistories');
            $InventoryTransactionHistoriesModel = FactoryLocator::get('Table')->get('InventoryTransactionHistories');
