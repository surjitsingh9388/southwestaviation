...
class PilotCheckingsTable extends Table
{
    ...
    public function beforeSave($options = array())
    {
        ...
        if (!empty($entity->AS_last_completed)) {
            $entity->AS_last_completed = $this->Planes->dateFormatBeforeSave($entity->AS_last_completed);
        }

        if (!empty($entity->ICC_last_completed)) {
            $entity->ICC_last_completed = $this->Planes->dateFormatBeforeSave($entity->ICC_last_completed);
        }

        if (!empty($entity->OW_last_completed)) {
            $entity->OW_last_completed = $this->Planes->dateFormatBeforeSave($entity->OW_last_completed);
        }

        if (!empty($entity->LC_last_completed)) {
            $entity->LC_last_completed = $this->Planes->dateFormatBeforeSave($entity->LC_last_completed);
        }

        if (!empty($entity->APC_last_completed)) {
            $entity->APC_last_completed = $this->Planes->dateFormatBeforeSave($entity->APC_last_completed);
        }

        if (!empty($entity->IO_last_completed)) {
            $entity->IO_last_completed = $this->Planes->dateFormatBeforeSave($entity->IO_last_completed);
        }

        if (!empty($entity->CAO_last_completed)) {
            $entity->CAO_last_completed = $this->Planes->dateFormatBeforeSave($entity->CAO_last_completed);
        }
        return true;
    }
