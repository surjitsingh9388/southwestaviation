...
class CustomerAircraftWOItemServicesTable extends Table
{
    ...
    public function beforeSave($options = array())
    {
        ...
        if (!empty($entity->time_start_datetime)) {
            $entity->time_start_datetime = $this->Planes->dateFormatBeforeSave($entity->time_start_datetime);
        }

        if(!empty($entity->id)){
            $CustomerAircraftWOItemServicesModel = TableRegistry::get('CustomerAircraftWOItemServices');
            $woitemservices = $CustomerAircraftWOItemServicesModel->get($entity->id);
            ...
            if($entity->service_add_time != $woitemservices->service_add_time){
                $description .= 'Add Time was changed from "'.$woitemservices->service_add_time.'" to "'.$entity->service_add_time.'".<br/>';
            }
