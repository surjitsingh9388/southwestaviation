...
class CustomerAircraftWOOSRInfoPOesTable extends Table
{
    ...
    public function beforeSave($options = array())
    {
        ...
        if (!empty($entity->date_order_placed)) {
            $entity->date_order_placed = $service->dateFormatBeforeSave($entity->date_order_placed);
        }
        if (!empty($entity->general_est_arrival_date)) {
            $entity->general_est_arrival_date = $service->dateFormatBeforeSave($entity->general_est_arrival_date);
        }

        if(!empty($entity->id)){
            $this->CustomerAircraftWOOSRInfoPOes =  FactoryLocator::get('Table')->get('CustomerAircraftWOOSRInfoPOes');
