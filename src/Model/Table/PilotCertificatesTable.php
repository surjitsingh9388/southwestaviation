...
class PilotCertificatesTable extends Table
{
    ...
    public function beforeSave($options = array())
    {
        ...
        if (!empty($entity->medical_last_completed)) {
            $entity->medical_last_completed = $service->dateFormatBeforeSave($entity->medical_last_completed);
        }

        if (!empty($entity->passport_last_completed)) {
            $entity->passport_last_completed = $service->dateFormatBeforeSave($entity->passport_last_completed);
        }

        if (!empty($entity->DL_last_completed)) {
            $entity->DL_last_completed = $service->dateFormatBeforeSave($entity->DL_last_completed);
        }

        if (!empty($entity->TPC_last_completed)) {
            $entity->TPC_last_completed = $service->dateFormatBeforeSave($entity->TPC_last_completed);
        }
        return true;
    }
    ...
    {
        $pilotCertificate = $this->newEmptyEntity();
        if (!empty($postData['medical_last_completed'])) {
            $postData['medical_last_completed'] = FrozenTime::createFromFormat('m/d/Y', $postData['medical_last_completed']);
        }

        if (!empty($postData['passport_last_completed'])) {
            $postData['passport_last_completed'] = FrozenTime::createFromFormat('m/d/Y', $postData['passport_last_completed']);
        }

        if (!empty($postData['DL_last_completed'])) {
            $postData['DL_last_completed'] = FrozenTime::createFromFormat('m/d/Y', $postData['DL_last_completed']);
        }

        if (!empty($postData['TPC_last_completed'])) {
            $postData['TPC_last_completed'] = FrozenTime::createFromFormat('m/d/Y', $postData['TPC_last_completed']);
        }
