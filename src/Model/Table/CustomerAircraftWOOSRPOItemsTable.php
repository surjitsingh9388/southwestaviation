...
class CustomerAircraftWOOSRPOItemsTable extends Table
{
    ...
    public function beforeSave($options = array())
    {
        ...
        if (!empty($entity->shelf_life)) {
            $entity->shelf_life = $service->dateFormatBeforeSave($entity->shelf_life);
        }
        ...
        if (!empty($entity->estimated_arrival_date)) {
            $entity->estimated_arrival_date = $service->dateFormatBeforeSave($entity->estimated_arrival_date);
        }
        return true;
    }
