...
class CustomerAircraftMaintenanceAdsTable extends Table
{
    ...
    public function beforeSave($options = array())
    {
        ...
        if(!empty($entity->ad_recurring_date)) {
            $entity->ad_recurring_date = $service->dateFormatBeforeSave($entity->ad_recurring_date);
        }
        if(!empty($entity->ad_revision_date)) {
            $entity->ad_revision_date = $service->dateFormatBeforeSave($entity->ad_revision_date);
        }
