...
function action(e){
    ...
    if(confirm(msg)){
        ...
        var ata_code_id = $(e).attr("data-ata_code_id");
        $("#actionForm").attr("action",url);
        $("#actionForm").append("<input type='hidden' name='id' value='"+ata_code_id+"'/>");
         $("#actionForm").submit();
     }
     return false;  
}
$(document).ready(function() {
    var dataTable = $('#datatable').DataTable( {
        ...
        "ajax":{
            url:"<?php echo $this->Url->build(['controller'=>'AtaCodes', 'action'=>'ajaxManageAtaCodesSearch']); ?>",
...
<div class="content sliding">
    <div class="outerWrapper">
        <div class="btnWrapper">
            <h2 class="heading">ATA Codes List</h2>
            <?php
            if((!empty($actionItems) && $actionItems['action']['action_add']==1) || $sessionUser['id'] == 1){
            echo $this->Html->link("<i class='fa fa-plus'></i> Add ATA Code", array('action' => 'add'), array('class' => 'btn btn-default', 'escape' => false));
            }
            ?>
        </div>
             ...
             <table id="datatable" class="dataTable table table-striped table-bordered table-responsive table-Width" width="100%">
                    <thead>
                        <tr>
                            ...
                            <th style="vertical-align: top;" scope="col"><?php echo __('ATA Code'); ?></th>
