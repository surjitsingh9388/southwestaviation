...
<tr class="activeTble">
    ...
    <td>
        <?php if(!empty($row['attachment_count'])){ ?>
    ...
    <td>
        ...
        if(!empty($row['nextDue']['mos']) || !empty($row['nextDue']['hrs']) || !empty($row['nextDue']['afl'])) {
            if(isset($row['remaining']['remMos']) && isset($row['remaining']['remDays']) && isset($row['remaining']['remHrs']) && ($row['remaining']['remMos'] == 0 && $row['remaining']['remDays'] == 0 && $row['remaining']['remHrs'] == 0 && $row['remaining']['remAfl'] == 0 && $row['remaining']['totalRemD'] == 0)) {
            ?>
                <div class="btn btnStatus not-due-status" <?php echo $alertColor;?>>
                    <?php echo $this->Html->link("0 Days", array('controller'=>'AirframeComponentParts', 'action'=>'edit', $row['partDet']['id'], $typ, $act), array('escape' => false)); ?>
