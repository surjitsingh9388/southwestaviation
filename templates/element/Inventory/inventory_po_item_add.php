<tr>
    ...
    <td>
        ...
        if(isset($invreqitem['invitms']['name']) || (isset($invtype) && $invtype == 'invitem')){
            ...
            $inventoryitemdropdown .= '<input type="hidden"  name="noninventory_item[]"><span class="input-group-btn"><button class="btn btn-primary addnewinvitempopup" type="button" tabindex="-1"><i class="fa fa-plus"></i></button><button class="btn btn-primary searchInventoryItemPopup" type="button" tabindex="-1"><i class="fa fa-search"></i></button></span>';
        }else{
    ...
    <td>
        ...
        $uomdropdown = '<select name="uom[]" class="form-control col-md-8 col-xs-12 selectpicker invitem_default_uom" aria-label="Default select example" required="required">';
        foreach($defaultUOM as $key=>$val){
            if(isset($invtype) && $invtype == 'invitem'){
                $selected = (isset($invreqitem->default_uom) && $invreqitem->default_uom == $key) ? 'selected' : '';
            }else{
                $selected = (isset($invreqitem->uom) && $invreqitem->uom == $key) ? 'selected' : '';
            }
